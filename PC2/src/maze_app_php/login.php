...
// 3. Consulta — devolve também Tipo e IDUtilizador (controlam privilégios depois)
$sql = "SELECT IDUtilizador, Equipa, Tipo FROM utilizador WHERE Email = ?";
...
if ($stmt) {
    ...
    if ($user) {
        ...
        $response['IDGrupo']      = (string)$user['Equipa'];
        $response['IDUtilizador'] = (string)$user['IDUtilizador'];
